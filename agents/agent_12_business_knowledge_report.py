...
def generate(
    ...
    report_data = {
        ...
        "confidence_score_median": _percentile(confidence_values, 0.5) if confidence_values else None,
    ...
    confidence_median_value = f'{report_data["confidence_score_median"]:.1f}%' if confidence_values else "—"
    confidence_median_detail = (
        f'{len(confidence_values)} independently scored rule{"s" if len(confidence_values) != 1 else ""}'
        if confidence_values else "No independently scored confidence values"
    )
