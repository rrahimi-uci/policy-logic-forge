...
def _readiness_definition_html() -> str:
    ...
    return (
        '<div class="panel score-definition"><div class="eyebrow">Scoring definition</div>'
...
.readiness-score{{min-width:180px}}.score-number{{font-size:25px;font-weight:850;letter-spacing:-.03em}}.score-number span{{font-size:11px;color:var(--muted);letter-spacing:0;margin-left:3px}}.score-track,.score-factor-track{{display:block;height:8px;border-radius:99px;background:#e9edf5;overflow:hidden}}.score-track{{margin:5px 0 8px}}.score-track i,.score-factor-track i{{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,var(--violet),var(--teal))}}.score-factors{{display:grid;gap:7px;margin-top:9px}}.score-factor{{display:grid;grid-template-columns:minmax(115px,1fr) 38px;gap:4px 8px;align-items:center;font-size:11px}}.score-factor strong{{text-align:right}}.score-factor-track{{grid-column:1/-1;height:5px}}.score-definition code{{display:inline-block;margin-top:6px;padding:7px 10px;border-radius:8px;background:#f4f6fb;color:#4251bd;font:12px/1.45 ui-monospace,SFMono-Regular,Menlo,monospace}}.score-definition .table-wrap{{max-height:none;margin:16px 0}}.score-definition th:nth-child(2),.score-definition td:nth-child(2){{white-space:nowrap;text-align:center}}.score-interpretation{{margin-top:14px}}
 </style></head><body><div class="shell">
<header class="hero"><div class="eyebrow">Presentation &amp; Knowledge Exploration</div><h1>Business knowledge, made transparent.</h1><p>A self-contained, source-traceable view of the extracted domain knowledge. Every rule is linked to its source and assigned a neutral <strong>0–100 Automation Readiness Score</strong>, providing a consistent measure of its suitability for automation. Your environment, risk tolerance, and domain policies determine the appropriate acceptance threshold.</p><div class="muted" style="color:#cbd9ee">Generated from {_safe(graph_file.name)} · no external assets or network calls required</div></header>
<div class="metric-grid">{_metric_card('Business rules', len(rules), 'All extracted rules shown')}{_metric_card('Average readiness score', f'{report_data["automation_readiness_score_average"]:.1f}/100', 'No report-defined pass threshold')}{_metric_card('Score range', f'{report_data["automation_readiness_score_minimum"]:.1f}–{report_data["automation_readiness_score_maximum"]:.1f}', 'Observed across all rules')}{_metric_card('SBVR concepts', concept_supported, f'{vocabulary_coverage:.1f}% with concept-specific evidence')}{_metric_card('Decision variables', len(decision_variables), 'Executable symbols kept outside SBVR')}{_metric_card('Contract integrity', f'{grounding_dimension_metrics["contract"]["support_rate"]:.1f}%', 'Derived structural projections')}{_metric_card('Relationship support', f'{grounding_dimension_metrics["relationship"]["support_rate"]:.1f}%', 'Graph relationship grounding')}{_metric_card('Source pointers', f'{source_pointer_count} ({report_data["source_pointer_coverage_rate"]:.1f}%)', 'Pointer presence, not proof')}{_metric_card('Grounding claims', f'{grounding_claim_counts.get("supported", 0)} ({report_data["grounding_claim_support_rate"]:.1f}%)', 'Claim-level source support')}</div>
