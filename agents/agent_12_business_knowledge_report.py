...
def _validate_report_html(document: str, rule_ids: Sequence[str], source_anchors: Sequence[str]) -> None:
    ...
    required = (
        ...
        'data-tab="overview"', 'data-tab="vocabulary"', 'data-tab="rules"', 'data-tab="dependencies"', 'data-tab="sources"',
...
<div class="metric-grid">{_metric_card('Total business rules', len(rules), 'All extracted rules shown')}{_metric_card('Average and median readiness score', f'{report_data["automation_readiness_score_average"]:.1f} / {report_data["automation_readiness_score_median"]:.1f}', 'Average / median, each out of 100')}{_metric_card('10th-percentile readiness score', f'{report_data["automation_readiness_score_p10"]:.1f}/100', '90% of rules score at or above this value')}{_metric_card('Grounding claim support', f'{report_data["grounding_claim_support_rate"]:.1f}%', f'{grounding_claim_counts.get("supported", 0)} of {grounding_claim_total} claims supported')}{_metric_card('Contract integrity', f'{grounding_dimension_metrics["contract"]["support_rate"]:.1f}%', 'Rules with certified contract projections')}{_metric_card('Relationship support', f'{grounding_dimension_metrics["relationship"]["support_rate"]:.1f}%', 'Rules with supported graph relationships')}<div class="metric-card threshold-card"><div id="threshold-rule-count" class="metric-value">—</div><div class="metric-label">Rules below the user-selected threshold</div><div id="threshold-rule-rate" class="metric-detail">Enter a threshold; none is assumed</div><label class="threshold-control"><span class="muted">Threshold</span><input id="readiness-threshold" type="number" min="0" max="100" step="0.1" placeholder="0–100" aria-label="User-selected readiness threshold"></label></div>{_metric_card('Contradicted and insufficient-evidence claims', grounding_claim_counts.get('contradicted', 0) + grounding_claim_counts.get('insufficient_evidence', 0), f'{grounding_claim_counts.get("contradicted", 0)} contradicted · {grounding_claim_counts.get("insufficient_evidence", 0)} insufficient evidence')}</div>
<nav class="tabs" aria-label="Report sections"><button class="active" data-tab="overview">Overview</button><button data-tab="vocabulary">SBVR vocabulary</button><button data-tab="rules">Rule explorer</button><button data-tab="dependencies">Relationships</button><button data-tab="sources">Source traceability</button></nav>
...
const readinessThreshold=document.getElementById('readiness-threshold'), thresholdRuleCount=document.getElementById('threshold-rule-count'), thresholdRuleRate=document.getElementById('threshold-rule-rate');
function updateThresholdMetric(){{const raw=readinessThreshold.value.trim();if(raw===''){{thresholdRuleCount.textContent='—';thresholdRuleRate.textContent='Enter a threshold; none is assumed';return;}}const threshold=Math.min(100,Math.max(0,Number(raw))),below=rows.filter(row=>Number(row.dataset.score||0)<threshold).length,rate=rows.length?(below/rows.length*100):0;thresholdRuleCount.textContent=String(below);thresholdRuleRate.textContent=rate.toFixed(1)+'% of '+rows.length+' rules below '+threshold.toFixed(1);}}
readinessThreshold.addEventListener('input',updateThresholdMetric);updateThresholdMetric();
