...
- Derivation is re-run as a *validation* pass at the end of `agent_07` and `agent_08` (`revalidate_graph`), because both rewrite variables in place. A relation whose acceptance condition no longer holds is dropped and reported rather than shipped with a stale attestation.
