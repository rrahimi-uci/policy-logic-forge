...
export function Overview({ run, stages, onStage, onView }: { run: RunSummary; stages: Stage[]; onStage: (stage: Stage) => void; onView: (view: string) => void }) {
  ...
  const humanReview = run.human_review_required_rules ?? run.queues?.human_review ?? 0;
  const humanReviewRate = run.human_review_rate ?? percent(humanReview, run.rule_count);
  ...
  return <div className="view-stack">
    ...
    <div className="metric-grid">
      ...
      <MetricCard label="Human-review queue" value={humanReview} detail={`${humanReviewRate}% of rules · quality holds remain separate`} tone={humanReview ? "warn" : "good"} />
...
/** BPMN is shown only when the source explicitly describes a process workflow. */
export function shouldRenderBpmn(rule: RuleDetail): boolean {
  // BPMN is a process notation, not a prettier rendering of every decision.
  // Only show it when the source-explicit workflow contract is complete enough
  // to produce an actual workflow model.  This mirrors the backend exporter
  // and prevents an illustrative BPMN diagram from being mistaken for source
  // evidence on simple rules.
  const workflow = rule.workflow_semantics;
  if (!workflow || workflow.kind !== "prescriptive_process" || workflow.basis !== "explicit_in_source") return false;
  if (!String(workflow.trigger_event || "").trim() || !String(workflow.actor_role || "").trim()) return false;
  if (workflow.actor_role !== String(rule.responsible_party || "").trim()) return false;
  if (!Array.isArray(workflow.evidence) || workflow.evidence.length === 0) return false;
  if (!Array.isArray(workflow.ordered_steps) || workflow.ordered_steps.length < 2) return false;
  const stepIds = new Set<string>();
  return workflow.ordered_steps.every((step: Record<string, any>) => {
    const id = String(step?.step_id || "").trim();
    if (!id || stepIds.has(id) || !String(step?.name || "").trim()) return false;
    stepIds.add(id);
    return ["business_rule_task", "user_task", "service_task", "send_task", "receive_task"].includes(String(step?.kind));
  });
}
...
export function ExecutableRepresentations({ rule, onRule }: { rule: RuleDetail; onRule?: (ruleId: string) => void }) {
  ...
  const showBpmn = shouldRenderBpmn(rule);
  const [activeTab, setActiveTab] = useState<"dmn" | "bpmn" | "cmmn" | "sbvr">("dmn");
  useEffect(() => { setActiveTab("dmn"); }, [rule.rule_id]);
  const reviewRoute = rule.review_route?.route || "none";
  const cmmnRequired = reviewRoute === "case_management" || reviewRoute === "human_review";
  const sbvr = rule.sbvr_projection || {};
  const sbvrConcepts = Array.isArray(sbvr.concepts) ? sbvr.concepts : [];
  const sbvrFacts = Array.isArray(sbvr.fact_types) ? sbvr.fact_types : [];
  const tabs = [
    { id: "dmn" as const, label: "DMN", detail: "Decision table" },
    ...(showBpmn ? [{ id: "bpmn" as const, label: "BPMN", detail: "Explicit workflow" }] : []),
    { id: "cmmn" as const, label: "CMMN", detail: cmmnRequired ? "Review case" : "No case required" },
    { id: "sbvr" as const, label: "SBVR", detail: "Typed vocabulary" },
  ];
  return <section className="executable-representations"><div className="panel-heading compact"><div><p className="eyebrow">Executable projection</p><h3>Decision, workflow, case, and vocabulary views</h3></div><Badge value={rule.requires_review ? "requires_review" : hasDmn || (showBpmn && hasBpmn) ? "review_projection" : "not_projected"} /></div><div className="callout note" role="note">These are review projections generated from this rule&apos;s own contract fields, not proof-verified or executable artifacts. The pipeline&apos;s compiled, proof-checked decision layer (<code>lexec_ir.json</code>, <code>proof_records.json</code>) is a separate output covering only rules without contextual scope &mdash; see <code>docs/executable-models.md</code>.</div><div className="projection-tabs" role="tablist" aria-label="Executable model views">{tabs.map((tab) => <button key={tab.id} type="button" role="tab" aria-selected={activeTab === tab.id} aria-controls={`projection-panel-${tab.id}`} className={`projection-tab${activeTab === tab.id ? " active" : ""}`} onClick={() => setActiveTab(tab.id)}><span>{tab.label}</span><small>{tab.detail}</small></button>)}</div>{activeTab === "dmn" && <section id="projection-panel-dmn" className="executable-card projection-panel" role="tabpanel" aria-label="DMN decision table"><div className="executable-card-heading"><span className="projection-icon">DMN</span><div><strong>Decision table (review projection)</strong><small>{hasDmn ? `${String(dmn.hit_policy || rule.recommended_hit_policy || "UNIQUE")} hit policy declared · not proof-verified` : "No DMN metadata recorded · this view is always a review projection, never proof-verified"}</small></div></div><div className="dmn-table" role="table" aria-label="DMN decision table"><div className="dmn-row dmn-head" role="row"><span>#</span>{predicates.map((predicate, index) => <span key={`h-${index}`}>{String(predicate.variable || `Input ${index + 1}`)}</span>)}{outcomes.map((outcome, index) => <span key={`o-${index}`}>{String(outcome.variable || `Output ${index + 1}`)}</span>)}</div><div className="dmn-row" role="row"><span>1</span>{predicates.map((predicate, index) => <span key={`p-${index}`} title={`${String(predicate.operator || "condition")} ${JSON.stringify(predicate.value)}`}><b>{String(predicate.operator || "=")}</b> {JSON.stringify(predicate.value)}</span>)}{outcomes.map((outcome, index) => <span key={`v-${index}`}><b>{String(outcome.operator || "=")}</b> {JSON.stringify(outcome.value)}</span>)}</div></div><div className="connection-note">↳ Related rules: {relatedRuleIds.length ? relatedRuleIds.map((ruleId) => onRule ? <button className="link-button connection-link" key={ruleId} onClick={() => onRule(ruleId)}>{ruleId}</button> : <span className="mono" key={ruleId}>{ruleId}</span>) : <span>none recorded</span>}</div></section>}{activeTab === "bpmn" && showBpmn && <section id="projection-panel-bpmn" className="executable-card projection-panel" role="tabpanel" aria-label="BPMN workflow"><div className="executable-card-heading"><span className="projection-icon">BPMN</span><div><strong>Illustrative process diagram</strong><small>{hasBpmn ? `${String(bpmn.gateway_type || "exclusive")} gateway · lane ${String(bpmn.lane || rule.responsible_party || "unassigned")} · generated from the source-explicit workflow contract, not a simulated or executed process` : "Workflow contract is explicit; this diagram is illustrative, not a simulated or executed process"}</small></div></div><svg className="bpmn-diagram" viewBox="0 0 820 170" role="img" aria-label="BPMN workflow with start event, task, gateway, branches, and end events"><defs><marker id={`arrow-${rule.rule_id}`} markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="currentColor" /></marker></defs><circle cx="42" cy="82" r="18" className="bpmn-event" /><text x="42" y="122" textAnchor="middle">Start</text><path d="M60 82 H145" className="bpmn-flow" markerEnd={`url(#arrow-${rule.rule_id})`} /><rect x="145" y="52" width="180" height="60" rx="9" className="bpmn-task" /><text x="235" y="78" textAnchor="middle">Evaluate workflow</text><text x="235" y="96" textAnchor="middle" className="bpmn-subtext">{rule.rule_id.slice(0, 20)}</text><path d="M325 82 H392" className="bpmn-flow" markerEnd={`url(#arrow-${rule.rule_id})`} /><polygon points="420,55 447,82 420,109 393,82" className="bpmn-gateway" /><text x="420" y="133" textAnchor="middle">Decision</text><path d="M447 82 H560 V42 H660" className="bpmn-flow" markerEnd={`url(#arrow-${rule.rule_id})`} /><path d="M447 82 H560 V124 H660" className="bpmn-flow" markerEnd={`url(#arrow-${rule.rule_id})`} /><text x="565" y="36" className="bpmn-condition">match</text><text x="565" y="120" className="bpmn-condition">no match / review</text><circle cx="682" cy="42" r="18" className="bpmn-end" /><circle cx="682" cy="124" r="18" className="bpmn-end" /><text x="730" y="47">Outcome</text><text x="730" y="129">Review</text></svg></section>}{activeTab === "cmmn" && <section id="projection-panel-cmmn" className="executable-card projection-panel cmmn-card" role="tabpanel" aria-label="CMMN review case"><div className="executable-card-heading"><span className="projection-icon">CMMN</span><div><strong>{cmmnRequired ? "Review case guidance" : "No review case"}</strong><small>{cmmnRequired ? `Route: ${statusLabel(reviewRoute)} · human decision boundary preserved · guidance only, not an executable case lifecycle` : "This rule is machine-ready; CMMN is intentionally not instantiated."}</small></div></div>{cmmnRequired ? <div className="cmmn-case-flow"><div className="cmmn-stage"><span className="cmmn-stage-icon">1</span><div><strong>Review grounded findings</strong><small>Inspect source evidence and unresolved requirements.</small></div></div><span className="cmmn-arrow" aria-hidden="true">→</span><div className="cmmn-stage"><span className="cmmn-stage-icon">2</span><div><strong>Resolve and record</strong><small>Capture the reviewer disposition and rationale.</small></div></div><span className="cmmn-arrow" aria-hidden="true">→</span><div className="cmmn-stage"><span className="cmmn-stage-icon">3</span><div><strong>Review resolved</strong><small>Case closes only after evidence is reconciled.</small></div></div></div> : <div className="cmmn-empty"><span className="cmmn-check">✓</span><div><strong>Machine path is clear</strong><span>There is no source-grounded case-management task for this rule.</span></div></div>}</section>}{activeTab === "sbvr" && <section id="projection-panel-sbvr" className="executable-card projection-panel sbvr-card" role="tabpanel" aria-label="SBVR vocabulary"><div className="executable-card-heading"><span className="projection-icon">SBVR</span><div><strong>Typed vocabulary for this compliance package</strong><small>{sbvr.conformance === "derived_from_rule_contract" ? "Profile view derived from the validated rule contract · not a source quotation" : "SBVR-aligned semantic profile"}</small></div></div><div className="sbvr-grid"><section><h4>Concepts</h4>{sbvrConcepts.length ? <div className="sbvr-concepts">{sbvrConcepts.map((concept, index) => <article className="sbvr-concept" key={`${String(concept.concept_id || "concept")}-${index}`}><strong>{String(concept.preferred_term || concept.concept_id || "Unnamed concept")}</strong><span>{String(concept.concept_kind || "unresolved").replaceAll("_", " ")}{concept.role ? ` · ${String(concept.role)}` : ""}</span><small>{String(concept.basis || "unverified").replaceAll("_", " ")}</small></article>)}</div> : <div className="empty-state compact">No typed concepts were recorded for this rule.</div>}</section><section><h4>Fact type</h4>{sbvrFacts.length ? sbvrFacts.map((fact, index) => <article className="sbvr-fact" key={`${String(fact.fact_type_id || "fact")}-${index}`}><div className="sbvr-fact-line"><span>{String(fact.subject_concept || "subject")}</span><b>{String(fact.verb_term || "policy decision")}</b><span>{Array.isArray(fact.object_concepts) && fact.object_concepts.length ? fact.object_concepts.map(String).join(", ") : "decision outcome"}</span></div><div className="sbvr-fact-meta"><Badge value={String(fact.grounding_status || "unverified")} /><span>{Array.isArray(fact.source_evidence) ? `${fact.source_evidence.length} source link${fact.source_evidence.length === 1 ? "" : "s"}` : "No source links"}</span></div></article>) : <div className="empty-state compact">No fact type was recorded for this rule.</div>}</section></div></section>}</section>;
}
