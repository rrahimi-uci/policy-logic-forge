...
/** BPMN adds value when a rule has branching, multiple effects, exceptions, or dependencies. */
export function shouldRenderBpmn(rule: RuleDetail): boolean {
  return (rule.condition_predicates || []).length > 1
    || (rule.outcomes || []).length > 1
    || (rule.exceptions || []).length > 0
    || connectedRuleIds(rule.rule_id, rule.relationships || []).length > 0;
}
