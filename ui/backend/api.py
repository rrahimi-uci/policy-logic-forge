...
def _route_get(service: ReviewService, path: str, params: dict[str, list[str]]) -> Any | None:
    ...
    if segments[1:] == ["regdelta", "runs"]:
        return {"items": service.regdelta_runs.list_runs()}
    if segments[1:] == ["regdelta", "runs", "diff"]:
        old_run, new_run = _first(params, "old") or "", _first(params, "new") or ""
        if not old_run or not new_run:
            raise ValueError("both 'old' and 'new' run IDs are required")
        return service.regdelta_runs.diff(old_run, new_run)
