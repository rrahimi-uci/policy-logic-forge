...
## Follow-up: root cause found, fixed, and re-verified live

The "22 findings" pattern turned out not to be a Claude-specific
interpretation quirk at all. `agents/agent_02_entity_extractor.py`'s
`format_prompt("entity_extraction_compact", ...)` call — the prompt
`agent_02` actually sends, not the similarly-named but dead-code
`entity_extraction.txt` — never asked the model for the `source_evidence`
field its own `validate_catalog_evidence()` hard-requires. This was true
for **every** domain-prompt pack (all 5 generated by
`scripts/generate_benchmark_domain_prompts.py`), for **both** providers —
OpenAI runs likely only avoided this failure because GPT models happened
to volunteer an evidence-shaped field anyway despite it never being
requested, not because the contract was actually correct.

Fixed at the generator (`ENTITY_EXTRACTION_COMPACT`/`ENTITY_EXTRACTION` in
`scripts/generate_benchmark_domain_prompts.py`), then regenerated all 10
affected files (`entity_extraction[_compact].txt` × 5 domains) — the
`domain-prompts/*.txt` files are generated, not hand-edited, per this
repo's own convention. Also raised `entity_extractor.max_tokens`
8192 → 16384: once evidence quotes are correctly requested, the fuller
required output no longer reliably fits the old budget (confirmed via a
real `finish_reason=length` truncation on the first re-test, before this
second fix). New contract tests in `tests/test_data_contracts.py`
(`TestEntityExtractionCompactContract`) pin `source_evidence` — and its
`chunk_path`/`source_text` shape — against the file `agent_02` actually
requests, so this exact class of prompt/validator drift can't silently
ship again.

**Re-verified live against Claude after both fixes**, same source file,
same batch: Stage 01 (unaffected, still PASS), Stage 02 now PASSes
cleanly — 3 iterations, 10 entities extracted including all 5 of the
previously-failing categories (`FIRST_PARTY`, `USER`, `INFORMATION_TYPE`,
`COLLECTION_PRACTICE`, `THIRD_PARTY`), 15 relationships, 48 rule
summaries ($0.35) — followed by Stage 03 (Rules Extractor, PASS, 5 rules,
100% source-reference verification, $0.17) and Stages 04–05 (Validator,
Merger — no LLM calls in either, $0.00, confirmed provider-agnostic by
construction). The full core-extraction phase (Stages 01–05) is now
confirmed working end to end against a real Claude API key.

Stages 06–12 (optimizer, readiness, remediation, grounding, DAG
generation, executable models, the business report) were not re-verified
live against Anthropic in this pass — a reasonable next step, not
undertaken here given their larger scope and cost.
