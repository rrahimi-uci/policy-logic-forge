...
> **Provenance note.** This plan was written against the `policy-to-knowledge`
> monorepo's `apps/pipeline` before this repo (`compliance-to-code`) was split
> out of it as a focused fork. Its analysis, spike results, and file paths
> (e.g. `pipeline-output/fannie_mae_readiness_20260821/`, `apps/pipeline/...`)
> refer to that original repo's layout, not this one — see `README.md` for
> this repo's actual structure. The findings themselves carry over unchanged;
> three of the plan's flagged pre-existing defects (P2's contradictory
> extraction prompt, P3's stale `contract_issues` annotations, P6's
> mortgage-shaped BPMN gate) were fixed as part of this fork, since the P3/P6
> fixes are load-bearing for the four non-mortgage domains this repo actually
> carries (see the P6 entry below — it isn't hypothetical here, the four kept
> domains would otherwise get zero BPMN targets). P1's "almost no v2 corpus
> exists" and P4's "exception semantics are unspecified" are still open;
> Q1-Q5 at the end are all still genuinely open decisions.
